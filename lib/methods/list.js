...
function includeOptions(results, lists){
  var ret = {}
  let tmp = {}
  _.forEach(results, result=>{
    _.forEach(lists, list=>{
      tmp[list] = tmp[list] || {}
      _.forEach(result[list], listItem=>{
        if(!tmp[list][_.get(listItem, 'id')]){
          ret[list] = ret[list] || []
          tmp[list][_.get(listItem, 'id')] = true;
          ret[list].push(listItem)
        }
      })
    })
  })
    ...
    handle : function(  req,
      ...
      if(req.query[ "like" ]){
        ...
        _.each(filter, function(value,key){
          likeFilter[key] = new RegExp(escapeRegExp(value), 'i');
        });
        filter = likeFilter
      }
      ...
      query.exec = function( next ){
        query.count( function( err, count ){
          ...
          if(options.perPage){
            query.find().limit( resultsPerPage ).skip( skip ).populate( options.populate || config.populate );
          } else {
            query.find().populate( options.populate || config.populate );
          }
          ...
          query.originalExec( function( err, results ){
            ...
            if(options.perPage) {
              ...
              if(options.include){
                rtn.include = includeOptions(rtn.results, options.include)
              }
